...
pairwise_results_mean <- lapply(DOC_sum[, c(3, 5)], pairwise.t.test, DOC_sum$groups, p.adjust.methods = "bonferroni", pool.sd = TRUE)

oneway_test_results(DOC_sum, "mean_NPOC", "groups", log_normalise = TRUE)
statistics_pipeline_wrapper(DOC_sum, "mean_NPOC", "groups", log_normalise = TRUE)
...
oneway_test_results(DOC_sum, "var_NPOC", "groups", log_normalise = TRUE)
statistics_pipeline_wrapper(DOC_sum, "var_NPOC", "groups", log_normalise = TRUE)
...
DOM_averages <- data_sum[, .(FI2 = mean(FIX, na.rm = TRUE), HI2 = mean(HIX2, na.rm = TRUE),
                         beta.alpha2 = mean(beta.alpha, na.rm = TRUE), SUVA254_2 = mean(SUVA254, na.rm = TRUE),
                         SR = mean(SR_Loiselle, na.rm = TRUE), E2toE3 = mean(E2.to.E3, na.rm = TRUE),
                         C_humic = mean((Comp.1 + Comp.2 + Comp.3 + Comp.4 + Comp.5) / C_tot, na.rm = TRUE),
                         C_protein = mean((Comp.6 + Comp.7 + Comp.8) / C_tot, na.rm = TRUE),
                         DOC = mean(NPOC, na.rm = TRUE),
                         percent_HMWS_C = mean(HMWS_C / CDOC, na.rm = TRUE),
                         percent_Humic_C = mean(humic_like_substance_C / CDOC, na.rm = TRUE),
                         percent_LMWS_C = mean(LMWS_C / CDOC, na.rm = TRUE, trim = 5)),
...
log_normalise <- rep(FALSE, 12)
log_normalise[c(9, 10, 11)] <- TRUE

# calculate the oneway tset results of all the values (ir the flow regime tests have p < 0.05)
pairwise_results_mean <- lapply(DOM_averages[, -c(1:4)], pairwise.t.test, DOM_averages$groups.x, p.adjust.methods = "bonferroni", pool.sd = TRUE)
...
#  Create a log_normalise vector of logical values looking at the above histograms and shapiro results
log_normalise <- rep(FALSE, 12)
log_normalise[c(6, 7, 8, 11)] <- TRUE

# calculate the oneway test results of all the values (ir the flow regime tests have p < 0.05)
pairwise_results_CV <- lapply(DOM_CV_averages[, -c(1:4)], pairwise.t.test, DOM_CV_averages$groups.x, p.adjust.methods = "bonferroni", pool.sd = TRUE)
...
pairwise_results_mean <- lapply(PC_info[, c(2:5)], pairwise.t.test, DOC_sum$groups, p.adjust.methods = "bonferroni", pool.sd = TRUE)

oneway_test_results(PC_info, "mean_PC1", "groups", log_normalise = FALSE)
statistics_pipeline_wrapper(PC_info, "mean_PC1", "groups", log_normalise = FALSE)
...
oneway_test_results(PC_info, "var_PC1", "groups", log_normalise = FALSE)
statistics_pipeline_wrapper(PC_info, "var_PC1", "groups", log_normalise = FALSE)

# Mean of PC2
shapiro.test((PC_info$mean_PC2))
hist((PC_info$mean_PC2))

oneway_test_results(PC_info, "mean_PC2", "groups", log_normalise = FALSE)
statistics_pipeline_wrapper(PC_info, "mean_PC2", "groups", log_normalise = FALSE)
...
pairwise_results_var_PC2 <- lapply(log(PC_info[, c(5)]), pairwise.t.test, DOC_sum$groups, p.adjust.methods = "bonferroni", pool.sd = TRUE)

oneway_test_results(PC_info, "var_PC2", "groups", log_normalise = TRUE)
statistics_pipeline_wrapper(PC_info, "var_PC2", "groups", log_normalise = TRUE)
...
qqnorm(log(Multi_centroid$disp_PC_all), main = "Normal Q-Q Plot of male")
qqline((Multi_centroid$disp_PC_all))

oneway_test_results(Multi_centroid, "disp_PC_all", "groups", log_normalise = TRUE)
statistics_pipeline_wrapper(Multi_centroid, "disp_PC_all", "groups", log_normalise = TRUE)
...
pca_scores <- wine.pca$x
eigenvec12 <- cbind(wine.pca$rotation[, 1], wine.pca$rotation[, 2])
PCAloadings <- data.frame(cor(pca_data[, -(1:5)], pca_scores))
...
hull_data <- rbind(MedAlt, MedNat, TempAlt, TempNat)  #combine grp.a and grp.b
hull_data
...
hull_data$groups <- factor(hull_data$groups, levels = c("TempNat", "TempAlt", "MedNat", "MedAlt"))

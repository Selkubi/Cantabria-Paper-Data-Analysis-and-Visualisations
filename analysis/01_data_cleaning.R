...
sample_names <- as.data.table(read.csv("data/metafiles/sample_name_site_match.csv"))[,-c("NPOC", "BDOC")]
...
jun1 <- as.data.table(cbind(read.table("data/abs_data/may.txt", sep=",", header=TRUE), campaign="jun"))
...
abs_data <- rbind(oct1,feb1, apr1, jun1, aug1,dec1)
...
meta <- read.csv("data/metafiles/meta_file_2.csv")
meta[meta$campaign=="may",]$campaign <- "jun"

meta_sum_optical <- as.data.table(cbind(meta, data_means), key = c("site","campaign"))
...
# Updating the month name from may to june since this is the true sampling day
sample_loadings[sample_loadings$campaign=="may",]$campaign <- "jun"
sample_names[sample_names$campaign=="may",]$campaign <- "jun"
...
data_sum$campaign <- factor(data_sum$campaign, 
                            levels = c("feb", "apr", "jun", "aug", "oct",  "dec"), 
                            labels =  c("Feb", "Apr", "Jun", "Aug", "Oct", "Dec"))
meta_sum_optical <- meta_sum_optical[, -c(14, 15)]
meta_sum_optical$campaign <- factor(meta_sum_optical$campaign,
                                       levels = c("feb", "apr", "jun", "aug", "oct",  "dec"), 
                                       labels =  c("Feb", "Apr", "Jun", "Aug", "Oct", "Dec"))
...
data_sum$alteration_type_grouping <- factor(data_sum$alteration_type_grouping, 
                                            levels = c("TempNat", "TempAlt_irrigation", "TempAlt_hydropower", "MedNat", "MedAlt_irrigation"), 
                                            labels = c("natural Atlantic", "altered (irrigation) Atlantic", "altered (hydropower) Atlantic", 

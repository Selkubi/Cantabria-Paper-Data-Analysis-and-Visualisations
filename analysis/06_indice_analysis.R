...
for (i in seq_along(data[, -c(1:4)])) {
  ...
  indice_results[[i]] <- statistics_pipeline_wrapper(data, names(data[, -c(1:4)])[i], "groups", log_normalise = TRUE)
}

bartlett.test((nPLow) ~ groups, data = data)
oneway.test((nPLow) ~ groups, var.equal = FALSE, data = data)

var.test((nPLow) ~ alteration, data = data[Class == "Mediterranean"])
t.test((nPLow) ~ alteration, data = data[Class == "Mediterranean"], var.equal = TRUE)

var.test((nPLow) ~ alteration, data = data[Class == "Temperate"])
t.test((nPLow) ~ alteration, data = data[Class == "Temperate"], var.equal = TRUE)

var.test((nPLow) ~ Class, data = data[alteration == "Natural"])
t.test((nPLow) ~ Class, data = data[alteration == "Natural"], var.equal = FALSE)

p.adjust(c(0.001737,  0.4524,   0.603), method = "bonferroni", n = 3)

m <- pairwise.t.test((data$nPLow), data$groups, p.adjust.method = "bonferroni", pool.sd = FALSE)
...
model_varPC2 <- plsr(var_PC2 ~ ., data = data[, -c("site", "Class", "alteration", "groups.x", "mean_PC1", "mean_PC2", "var_PC1", "groups")], scale = TRUE, validation = "CV")
summary(model_varPC2)

data2 <- merge(Multi_centroid[, -c(3:13, 17:22)], meta_indices, by.x = "site", by.y = "Stream")

model_dispPC_all <- plsr(disp_PC_all ~ ., data = data2[, -c("site", "Class", "alteration", "groups")], scale = TRUE, validation = "CV")
...
for (i in seq_along(length(plot_indexes$index))){

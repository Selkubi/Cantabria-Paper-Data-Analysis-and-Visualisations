...
data_sum2 <- data_sum %>%
  select(campaign, site, Comp1, Comp2, Comp3, Comp4, Comp5, Comp6, Comp7, Comp8, BDOC.x, SUVA254, 
         ...
         E2.to.E3, E4.to.E6, DecAbsCoeff254, groups)
BDOC_normalised_components<- data_sum2 %>%
  ...
  select(starts_with("Comp"),BDOC.x, SUVA254, 
         FIX, FIX2, HIX2, beta.alpha, DecAbsCoeff420, DecAbsCoeff430, DecAbsCoeff436, slope_classic, 
         E2.to.E3, E4.to.E6, DecAbsCoeff254, campaign, site, groups)
...
ggplot(data_sum, aes(x=wine.pca$x[,1], y=wine.pca$x[,2]))+
  geom_point(aes(color=data_sum$groups, fill=data_sum$groups), shape=21, size=2.5, colour="black")+
...
plot_TempAlt <- plot(wine.pca$x[,1:2], type="n", main = "TempAlt", ylim=c(-6,6), xlim=c(-6,6), cex.main=2, cex.axis=1.25)
ordi_TempAlt <- ordihull(ord=wine.pca$x[,1:2],groups=data_sum$site ,display="sites",draw="polygon", label=F, show.groups = group_list[[1]]$site,
...
ordi_TempNat <- ordihull(ord=wine.pca$x[,1:2],groups=data_sum$site ,display="sites",draw="polygon", label=F, show.groups = group_list[[2]]$site,
                         alpha=150, col=c("#4F9608"))
plot_MedAlt <- plot(wine.pca$x[,1:2], type="n", main = "MedAlt", ylim=c(-6,6), xlim=c(-6,6), cex.main=2, cex.axis=1.25)
ordi_MedAlt <- ordihull(ord=wine.pca$x[,1:2],groups=data_sum$site ,display="sites",draw="polygon", label=F, show.groups = group_list[[3]]$site,
                         alpha=150, col=c('#942D0A'))
plot_MedNat <- plot(wine.pca$x[,1:2], type="n", main = "MedNat", ylim=c(-6,6), xlim=c(-6,6), cex.main=2, cex.axis=1.25)
ordi_MedNat <- ordihull(ord=wine.pca$x[,1:2],groups=data_sum$site ,display="sites",draw="polygon", label=F, show.groups = group_list[[4]]$site,
...
for (i in 1:nrow(group_list[[1]])) {
  centroid_values [[i]] <- group_list[[1]][i,] %>%
      mutate(PC2=centroid(ordi_TempAlt[[i]])[1],
            PC1=centroid(ordi_TempAlt[[i]])[2])
}
centroid_TempAlt <- do.call(rbind.data.frame, centroid_values)

#TempNat
centroid_values <- list()
for (i in 1:(nrow(group_list[[2]]))) {
  centroid_values [[i]] <- group_list[[2]][i,] %>%
...
#MedAlt
centroid_values <- list()
for (i in 1:nrow(group_list[[3]])) {
  centroid_values [[i]] <- group_list[[3]][i,] %>%
    mutate(PC2=centroid(ordi_MedAlt[[i]])[1],
           PC1=centroid(ordi_MedAlt[[i]])[2])
}
centroid_MedAlt <- do.call(rbind.data.frame, centroid_values)

#MedNat
centroid_values <- list()
for (i in 1:(nrow(group_list[[4]]))) {
  centroid_values [[i]] <- group_list[[4]][i,] %>%
...
centoid_data <- rbind(centroid_TempAlt,centroid_TempNat, centroid_MedAlt, centroid_MedNat)
...
ordi_data <- append(ordi_TempAlt, values=c(ordi_TempNat, ordi_MedAlt, ordi_MedNat)) #combines the ordi point lists as a 1 list to be used in the loop later
...
data_anova <- site_info %>%
  ...
  select(mean_centroid_distance,groups)

anova(lm(data_anova$mean_centroid_distance ~ factor(data_anova$groups)))

disp<-tapply(betadisper(vegdist(wine.pca$x,method="euclidean"),group=BDOC_normalised_components$site)$distances,BDOC_normalised_components$site,mean)
boxplot(disp~BDOC_normalised_components$groups[match(names(disp),BDOC_normalised_components$site)], 
        xlab=NULL, ylab = "Dispersion of Qualitative DOM measures", col=c('#942D0A','#E65525', '#043005', "#4F9608"), main="Variance in DOM quality over the seasons")

# Permanova on groups
adonis(as.numeric(data_anova$mean_centroid_distance)~ factor(data_anova$groups), data=data_anova, method="euclidian", perm=999)
...
ordi_data2$groups <- as_factor(ordi_data2$groups)


ordi_data2 %>%
  filter(Class=="Temperate") %>%
  ggplot() +
  geom_boxplot(aes(x=reorder(site, as.numeric(groups)), y=Distances, fill=groups))+
...
j <- 1
  ...
  for(i in seq_along(group_list[[j]][[1]])){
    plot[[i]] <- ggplot(flow_data) + 
      ...
      geom_line(data=subset(data_sum3, site==group_list[[j]][[1]][[i]]), aes(x=exact_date, y=Comp6*scale_factor), color="red")+
...
#} 
 ...
 pca_data2 <- BDOC_normalised_components%>% #or put ddata_sum2 
   select(starts_with("Comp"), site, campaign)
 
 wine.pca <- prcomp(pca_data2[,-(9:10)], scale. = TRUE) 
 data_sum4 <- cbind(wine.pca$x[,1:2], as.data.frame(pca_data2[,c("site","campaign")]))

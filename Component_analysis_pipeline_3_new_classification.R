...
data_sum2 <- data_sum %>%
  select(campaign, site, Comp1, Comp2, Comp3, Comp4, Comp5, Comp6, Comp7, Comp8, BDOC, NPOC, SUVA254, 
         ...
         SR_Helms, SR_Loiselle, E2.to.E3, E4.to.E6, DecAbsCoeff254, groups.x, new_design, new)
...
BDOC_normalised_components<- data_sum2 %>%
  ...
  select(starts_with("Comp"),NPOC, SUVA254, 
         FIX, FIX2, HIX2, beta.alpha, slope_classic, slope_lm, slope_short_Helms, slope_short_Loiselle,
         SR_Helms, SR_Loiselle, E2.to.E3, E4.to.E6, DecAbsCoeff254, campaign, site, groups.x, new_design, new)
...
NPOC_var_plot <- ggplot(DOC_variance_sum) +
  ...
  coord_flip()+
  labs(title="Seasonal NPOC variance per hydrological class", subtitle = "December is NaN")
#dev.new()
#png('Seasonal NPOC variance per hydrological class')
#NPOC_var_plot
#dev.off()
...
for(i in seq_along(group_types)){
  group_list[[i]] <- site_info %>%
    select(site, Class, alteration, groups, Alteration_type, Catchment, new)%>%
    filter(groups==group_types[[i]]) %>%
    unique()
}
new_group_types <- factor(c("SouthAlt", "SouthNat", "NorthAlt", "NorthNat"))

group_list_new <- vector("list")
for(i in seq_along(new_group_types)){
  group_list_new[[i]] <- site_info %>%
    select(site, Class, alteration, groups, Alteration_type, Catchment, new)%>%
    filter(new==new_group_types[[i]]) %>%
    unique()
}
...
ggplot(data_sum, aes(x=wine.pca$x[,1], y=wine.pca$x[,2]))+
  geom_point(aes(color=data_sum$new, fill=data_sum$new), shape=21, size=4, colour="black")+
...
ordi_TempAlt <- ordihull(ord=wine.pca$x[,c(1,2)],groups=data_sum$site ,display="sites",draw="polygon", label=F, show.groups = group_list_new[[1]]$site,
...
ordi_TempNat <- ordihull(ord=wine.pca$x[,c(1,2)],groups=data_sum$site ,display="sites",draw="polygon", label=F, show.groups = group_list_new[[2]]$site,
...
ordi_MedAlt <- ordihull(ord=wine.pca$x[,c(1,2)],groups=data_sum$site ,display="sites",draw="polygon", label=F, show.groups = group_list_new[[3]]$site,
...
ordi_MedNat <- ordihull(ord=wine.pca$x[,c(1,2)],groups=data_sum$site ,display="sites",draw="polygon", label=F, show.groups = group_list_new[[4]]$site,
...
boxplot(disp[["value"]]~BDOC_normalised_components$new[match(disp[[1]],BDOC_normalised_components$site)], 
...
unique_data<-site_info[,c("site", "groups","new")]
disp<-left_join(disp, as_tibble(unique_data), by=c("site"="site"))

disp_med<-disp[str_sub(disp$new, 1,5) == "South",]
disp_temp<-disp[str_sub(disp$new, 1,5) =="North",]

anova(lm(disp_med[["value"]]~factor(disp_med$new)))
anova(lm(disp_temp[["value"]]~factor(disp_temp$new)))
...
distance_matrix %>% 
  filter(new=="NorthAlt" | new=="NorthNat") %>%
  ggplot() +
  geom_boxplot(aes(x=reorder(site, distances, mean), y=distances, fill=new))+
  theme_classic()+ coord_flip()+
  labs(color="New Class", x="Sites", y="Distance to centroid", title="Average Distance to Centroid")+
...
ggplot(data_sum, aes(x=wine.pca$x[,1], y=wine.pca$x[,2]))+
  #geom_point(aes(color=data_sum$groups.x, fill=data_sum$groups.x), shape=21, size=3, colour="black")+
  #scale_fill_manual(values=c("#E65525", "#942D0A", "#043005","#4F9608"))+
  #geom_segment(data = LC_OCD_rot, aes(x = 0, y = 0, xend = (PC1*15), yend = (PC2*15)), arrow = arrow(length = unit(1/2, "picas")),color = "black") +
  #annotate("text", x = (LC_OCD_rot$PC1*15), y = (LC_OCD_rot$PC2*15), label = LC_OCD_rot$Variables)+
...
head(meta_indices)
data_sum6 <- data_sum5 %>% arrange(site) %>%
  column_to_rownames("site")

meta_indices2 <- meta_indices %>% arrange(Stream) %>% 
  left_join(data_sum5, by=c("Stream"="site")) %>%
...
coul2 <- colorRampPalette(brewer.pal(9, "RdBu"))(10)
